...
#### Last modified:
#### 06/26/2025 changed error check for invertible matrix
#### 06/18/2025 changed what is returned from the function to include draws of other parameters
############################################
#### Inputs:
...
####    betadraws -- draws of regression coefficients of Y|Z,S=0
####    residdraws --- draws of residual error variance of Y|Z,S=0
####    phidraws --- draws of phi
####    rhodraws --- draws of Cor(Y,X|Z,S=1) [conditional proxy strength]
############################################
...
mub_reg_bayes <- function(stats_selected, stats_not_selected, zparams, userphi=NA, ndraws=1500)
{
  ...
  ###############
  DRAWS_beta_YZA.ZA_s <- apply(matrix(DRAWS_var_Y.ZA_s),1, function(s) mnormt::rmnorm(mean=beta_YZA.ZA_s, varcov=s*mult.mat.za.s))
    ...
    while(pd == 0)  # Checking draws to ensure positive-definite covariance matrix
    {
      ...
      while (smat_invertible == 0)  # Checking to ensure residual covariance matrix (X,Y|Z,S=1) is invertible
      {
        ...
        if (!is.matrix(check))
        {
          ...
          DRAWS_beta_YZA.ZA_s[,d] <- mnormt::rmnorm(mean=beta_YZA.ZA_s, varcov=DRAWS_var_Y.ZA_s[d]*mult.mat.za.s)
        } else if (is.matrix(check))
        {
          smat_invertible <- 1
        }
      }
      ...
      ###############
      DRAW_var_XY.Z_s <- MCMCpack::riwish(n_s-zparams-1, Smat_s_d) * (n_s-zparams-1)
      ...
      DRAW_coef.s <- mnormt::rmnorm(mean=c(beta_XZ.Z_s_d,beta_YZ.Z_s), varcov=coef.vc.s_d)
      ...
      DRAW_coef.ns <- mnormt::rmnorm(mean=beta_XZ.Z_ns_d, varcov=coef.vc.ns_d)
